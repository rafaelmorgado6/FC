...
for i=1:length(t)-1
    v(i+1)=v(i)-k*x(i)/m*h; %expressao da velocidade
    x(i+1)=x(i)+v(i)*h;     %expressao da posição
end
...
plot(t,v)
grid on
title('Método de Euler velocidade em funçao do tempo')
xlabel('tempo (s)')
...
plot(t,x)
grid on
title('Método de Euler posição em funçao do tempo')
xlabel('tempo (s)')
ylabel('Posição (m)')
...
plot(x,v)
grid on
title('Método de Euler velocidade em funçao da posição')
xlabel('Posição (m)')
ylabel('Velocidade (m/s)')
...
plot(t,Em)
grid on
title('Euler => Energia mecânica em função do tempo')
xlabel('tempo (s)')
