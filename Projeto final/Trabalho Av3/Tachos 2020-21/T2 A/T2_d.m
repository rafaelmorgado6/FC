...
% Energia aproximada p/ o nível 3
nivel = 3;
fprintf('\n')
...
% Cálculo exato da Energia
syms y
b = E_aprox/(-2^(1/3));
zeroAi = vpasolve(airy(y) == 0, y, b);

E_exato = -2^(1/3)*zeroAi;

A = psi_norm;
funcao = A.*airy(2^(-1/3)*(2*x - E_exato)); 
figure;
plot(x,funcao);
fprintf('E_exato calculado = %f Ha\n', E_exato)